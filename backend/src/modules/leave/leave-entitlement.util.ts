 ...
 *  - 1-5. kıdem yılları (5 dahil): yılda 14 gün
 *  - 6-14. kıdem yılları (5'ten fazla, 15'ten az): yılda 20 gün
 *  - 15. kıdem yılı ve sonrası (15 dahil): yılda 26 gün
 */
export function cumulativeAnnualLeaveEntitlement(hireDate: Date, asOf: Date = new Date()): number {
  ...
  const tier1 = Math.min(n, 5) * 14;
  const tier2 = Math.max(0, Math.min(n, 14) - 5) * 20;
